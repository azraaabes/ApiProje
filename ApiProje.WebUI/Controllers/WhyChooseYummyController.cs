...
using ApiProje.WebUI.Dtos.WhyChooseYummyDto;
...
namespace ApiProje.WebUI.Controllers
{
    public class WhyChooseYummyController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(createWhyChooseYummyDto); //biz bir metin göndericez bunu Api arafına kaydedebilmek için jsona dönüştürmemiz lazım,metinten jsona dönüştürürken serialize yazıyoruz.
            ...
            var responseMessage = await client.PostAsync("https://localhost:7092/api/Services", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7092/api/Services?id=" + id);
            return RedirectToAction("WhyChooseYummyList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7092/api/Services/GetService?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateWhyChooseYummy(UpdateWhyChooseYummyDto updateWhyChooseYummyDto)
        {
            ...
            await client.PutAsync("https://localhost:7092/api/Services/", stringContent);
            return RedirectToAction("WhyChooseYummyList");
        }
    }
}
