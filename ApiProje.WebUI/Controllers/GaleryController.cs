using ApiProje.WebUI.Dtos.ImageDtos;
using ApiProje.WebUI.Dtos.ImageDtos;
...
namespace ApiProje.WebUI.Controllers
{
    public class GaleryController : Controller
    {
        ...
        public async Task<IActionResult> ImageList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7092/api/Images");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var jsonData = JsonConvert.SerializeObject(createImageDto); //biz bir metin göndericez bunu Api arafına kaydedebilmek için jsona dönüştürmemiz lazım,metinten jsona dönüştürürken serialize yazıyoruz.
            ...
            var responseMessage = await client.PostAsync("https://localhost:7092/api/Images", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7092/api/Images?id=" + id);
            return RedirectToAction("ImageListWithEdit");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7092/api/Images/GetImage?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateImage(UpdateImageDto updateImageDto)
        {
            ...
            await client.PutAsync("https://localhost:7092/api/Images/", stringContent);
            return RedirectToAction("ImageListWithEdit");
        }
    }
}
